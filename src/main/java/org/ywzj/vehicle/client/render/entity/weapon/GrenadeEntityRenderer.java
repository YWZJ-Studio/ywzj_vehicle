...
public class GrenadeEntityRenderer extends EntityRenderer<GrenadeEntity> {
    ...
    @Override
    public boolean shouldRender(GrenadeEntity pLivingEntity, Frustum pCamera, double pCamX, double pCamY, double pCamZ) {
        return true;
    }
    ...
    public void render(GrenadeEntity pEntity, float pEntityYaw, float pPartialTick, PoseStack pPoseStack, MultiBufferSource bufferSource, int pPackedLight) {
        pPoseStack.pushPose();
        {
            ...
            if (model == null) {
                model = BedrockModelLoader.getModel(YwzjVehicle.modLocation("entity/grenade_40mm"));
            }
            if (builder == null) {
                builder = bufferSource.getBuffer(RenderType.entityCutout(YwzjVehicle.modLocation("textures/entity/grenade_40mm.png")));
            }
