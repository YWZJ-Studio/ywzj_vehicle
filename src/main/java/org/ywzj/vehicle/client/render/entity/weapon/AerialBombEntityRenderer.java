...
public class AerialBombEntityRenderer extends EntityRenderer<AerialBombEntity> {
    ...
    @Override
    public boolean shouldRender(AerialBombEntity pLivingEntity, Frustum pCamera, double pCamX, double pCamY, double pCamZ) {
        return true;
    }
    ...
    public void render(AerialBombEntity pEntity, float pEntityYaw, float pPartialTick, PoseStack pPoseStack, MultiBufferSource bufferSource, int pPackedLight) {
        pPoseStack.pushPose();
        {
            ...
            if (builder == null) {
                builder = bufferSource.getBuffer(RenderType.entityCutout(YwzjVehicle.modLocation("textures/entity/aerial_bomb.png")));
            }
