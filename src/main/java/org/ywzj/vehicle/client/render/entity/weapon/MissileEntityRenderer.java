...
public class MissileEntityRenderer extends EntityRenderer<MissileEntity> {
    ...
    @Override
    public boolean shouldRender(MissileEntity pLivingEntity, Frustum pCamera, double pCamX, double pCamY, double pCamZ) {
        return true;
    }
    ...
    public void render(MissileEntity pEntity, float pEntityYaw, float pPartialTick, PoseStack pPoseStack, MultiBufferSource bufferSource, int pPackedLight) {
        pPoseStack.pushPose();
        {
            ...
            if (model == null) {
                model = BedrockModelLoader.getModel(YwzjVehicle.modLocation("entity/missile_akd10"));
            }
            if (builder == null) {
                builder = bufferSource.getBuffer(RenderType.entityCutout(YwzjVehicle.modLocation("textures/entity/missile_akd10.png")));
            }
