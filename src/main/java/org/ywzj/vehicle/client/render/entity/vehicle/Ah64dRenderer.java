...
import org.ywzj.vehicle.entity.vehicle.Ah64d;
import org.ywzj.vehicle.vehicle.parts.PartUnit;
import org.ywzj.vehicle.vehicle.parts.WeaponUnit;

public class Ah64dRenderer extends EntityRenderer<Ah64d> {

    public Ah64dRenderer(EntityRendererProvider.Context pContext) {
        super(pContext);
    }

    @Override
    public void render(Ah64d pEntity, float pEntityYaw, float pPartialTick, PoseStack pPoseStack, MultiBufferSource bufferSource, int pPackedLight) {
        var display = ClientAssetsManager.INSTANCE.getVehicleDisplay(AllEntities.AH64D.getId()).orElse(null);
        ...
        BedrockBone propeller = model.getBoneMap().get("propeller");
        BedrockBone propeller2 = model.getBoneMap().get("propeller_tail");
        BedrockBone cannon = model.getBoneMap().get("mg");
        BedrockBone cannonElevation = model.getBoneMap().get("mg_up");
        ...
        cannon.rotation.mul(Axis.YN.rotationDegrees(turretYRot));
        ...
        cannonElevation.rotation.set(reset);

        pPoseStack.popPose();
    }

    @Override
    public ResourceLocation getTextureLocation(Ah64d pEntity) {
        return null;
    }
