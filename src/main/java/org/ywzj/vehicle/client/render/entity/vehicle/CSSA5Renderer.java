...
import org.ywzj.vehicle.entity.vehicle.Cssa5;
import org.ywzj.vehicle.vehicle.parts.PartUnit;
import org.ywzj.vehicle.vehicle.parts.RadarUnit;
import org.ywzj.vehicle.vehicle.parts.WeaponUnit;

public class CSSA5Renderer extends EntityRenderer<Cssa5> {

    public CSSA5Renderer(EntityRendererProvider.Context pContext) {
        super(pContext);
    }

    @Override
    public void render(Cssa5 pEntity, float pEntityYaw, float pPartialTick, PoseStack pPoseStack, MultiBufferSource bufferSource, int pPackedLight) {
        pPoseStack.pushPose();
        ...
        var display = ClientAssetsManager.INSTANCE.getVehicleDisplay(AllEntities.CSSA5.getId()).orElse(null);
        ...
        BedrockBone wheel4 = model.getBoneMap().get("wheel5");
        BedrockBone wheel5 = model.getBoneMap().get("wheel4");
        ...
        BedrockBone cannon = model.getBoneMap().get("machine_gun");
        BedrockBone missileLeft = model.getBoneMap().get("missile_left");
        BedrockBone missileRight = model.getBoneMap().get("missile_right");
        BedrockBone radar = model.getBoneMap().get("obj_center");
        BedrockBone gunBarrels = model.getBoneMap().get("machine_gun_rotator");

        // 轮子转速
        float vf = pEntity.getEntityData().get(Cssa5.FORWARD_SPEED);
        ...
        // 轮子转向幅度
        float vt = pEntity.getEntityData().get(Cssa5.TURN_ANGLE);
        ...
        if (!pEntity.seats.isEmpty()) {
            ...
            if (partUnit instanceof WeaponUnit weaponUnit) {
                ...
                WeaponUnit missile = weaponUnit.getSubWeaponUnits().get(0);
                missileXRot = -Mth.rotLerp(pPartialTick, missile.xRotO, missile.getXRot());
            }
        }
        ...
        missileRight.rotation.mul(Axis.XN.rotationDegrees(missileXRot));
        ...
        radar.rotation.set(reset);
        gunBarrels.rotation.set(reset);
        missileLeft.rotation.set(reset);
        missileRight.rotation.set(reset);

        pPoseStack.popPose();
    }

    @Override
    public ResourceLocation getTextureLocation(Cssa5 pEntity) {
        return null;
    }
