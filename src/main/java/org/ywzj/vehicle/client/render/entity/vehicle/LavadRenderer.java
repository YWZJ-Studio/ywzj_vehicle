...
import org.ywzj.vehicle.entity.vehicle.Lavad;
import org.ywzj.vehicle.vehicle.parts.PartUnit;
import org.ywzj.vehicle.vehicle.parts.WeaponUnit;

public class LavadRenderer extends EntityRenderer<Lavad> {

    public LavadRenderer(EntityRendererProvider.Context pContext) {
        super(pContext);
    }

    @Override
    public void render(Lavad pEntity, float pEntityYaw, float pPartialTick, PoseStack pPoseStack, MultiBufferSource bufferSource, int pPackedLight) {
        pPoseStack.pushPose();
        ...
        var display = ClientAssetsManager.INSTANCE.getVehicleDisplay(AllEntities.LAV_AD.getId()).orElse(null);
        ...
        BedrockBone turret = model.getBoneMap().get("turret_main");
        BedrockBone cannon = model.getBoneMap().get("mg");
        BedrockBone missile = model.getBoneMap().get("missile");
        BedrockBone gunBarrels = model.getBoneMap().get("mg_rotator");

        // 轮子转速
        float vf = pEntity.getEntityData().get(Lavad.FORWARD_SPEED);
        ...
        // 轮子转向幅度
        float vt = pEntity.getEntityData().get(Lavad.TURN_ANGLE);
        ...
        if (!pEntity.seats.isEmpty()) {
            ...
            if (partUnit instanceof WeaponUnit weaponUnit) {
                ...
                WeaponUnit missileWeaponUnit = weaponUnit.getSubWeaponUnits().get(0);
                missileXRot = -Mth.rotLerp(pPartialTick, missileWeaponUnit.xRotO, missileWeaponUnit.getXRot());
            }
        }
        ...
        missile.rotation.mul(Axis.XN.rotationDegrees(missileXRot));
        ...
        missile.rotation.set(reset);

        pPoseStack.popPose();
    }

    @Override
    public ResourceLocation getTextureLocation(Lavad pEntity) {
        return null;
    }
