    ...
    public void render(Z10 pEntity, float pEntityYaw, float pPartialTick, PoseStack pPoseStack, MultiBufferSource bufferSource, int pPackedLight) {
        ...
        if (!pEntity.seats.isEmpty()) {
            ...
            if (partUnit instanceof WeaponUnit weaponUnit) {
                WeaponUnit autoCannon = weaponUnit.getSubWeaponUnits().get(0);
                turretYRot = Mth.rotLerp(pPartialTick, autoCannon.yRotO, autoCannon.getYRot());
                turretXRot = Mth.rotLerp(pPartialTick, autoCannon.xRotO, autoCannon.getXRot());
                cameraYRot = Mth.rotLerp(pPartialTick, weaponUnit.yRotO, weaponUnit.getYRot());
                cameraXRot = Mth.rotLerp(pPartialTick, weaponUnit.xRotO, weaponUnit.getXRot());
            }
        }
        ...
        camera.rotation.mul(Axis.YN.rotationDegrees(cameraYRot + 180));
        camera.rotation.mul(Axis.XN.rotationDegrees(-cameraXRot));
