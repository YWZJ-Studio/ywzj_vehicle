    ...
    private static int setValue(CommandContext<CommandSourceStack> context) {
        ...
        if (context.getSource().getEntity() instanceof ServerPlayer serverPlayer) {
            if (DEBUG) {
                ...
                serverPlayer.sendSystemMessage(Component.literal("Vehicle Debug Mode is Turn On"));
            } else {
                Minecraft.getInstance().getEntityRenderDispatcher().setRenderHitBoxes(false);
                serverPlayer.sendSystemMessage(Component.literal("Vehicle Debug Mode is Turn Off"));
            }
        }
